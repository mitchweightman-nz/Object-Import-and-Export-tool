...
def setup_logging(log_widget):
    ...
    # Ensure log directory exists (optional, useful if logging to subfolder)
    ...
    # Use basicConfig to set root logger level and add handlers
    logging.basicConfig(level=log_level, handlers=[file_handler, console_handler, tkinter_handler])
    logging.info("Logging initialized.")
    # Start processing queue only if widget exists
...
def process_row(row_index, csv_data, mapping, default_location, username, selected_action,
    ...
    """
    Builds a single <node> element from CSV data plus UI overrides.
    Returns (node_element, None) on success, (None, error_message) on failure.
    """
    global global_docnum_counter
    std = {}
    meta_by_cat = {}
    try:
        # --- 1) Map from CSV columns ---
...
def run_processing(csv_file, xml_base, default_location, category, username,
    ...
    report_dict = None # Logic as before...
...
def save_reprocessed_nodes(reprocess_data, output_path):
    """Generates an XML file containing nodes marked for re-import. Input items are dicts with unique_id, generated_xml, action_state."""
...
class Application(tk.Tk):
    # ... (__init__ and other methods mostly) ...
    def __init__(self):
        super().__init__()
        self.title("OI Import Generator (v2.4.1 Reprocess Update)") # Version bump
        ...
        log_scroll_y = tk.Scrollbar(self.log_frame, orient="vertical", command=self.log_text.yview)
        log_scroll_x = tk.Scrollbar(self.log_frame, orient="horizontal", command=self.log_text.xview)
    ...
    # ... (Keep load_config_values, gather_current_config_dict) ...
    ...
    def create_widgets(self):
        ...
        self.settings_frame = tk.Frame(self.notebook); self.notebook.add(self.settings_frame, text="Settings"); self.create_settings_tab(self.settings_frame)
        self.mapping_frame = tk.Frame(self.notebook); self.notebook.add(self.mapping_frame, text="CSV Mapping"); self.create_csv_mapping_tab(self.mapping_frame)
        self.categories_frame = tk.Frame(self.notebook); self.notebook.add(self.categories_frame, text="Categories"); self.create_categories_tab(self.categories_frame)
        self.special_frame = tk.Frame(self.notebook); self.notebook.add(self.special_frame, text="Special Mapping"); self.create_special_mapping_tab(self.special_frame)
        self.reprocess_frame = tk.Frame(self.notebook); self.notebook.add(self.reprocess_frame, text="Reprocess"); self.create_reprocess_tab(self.reprocess_frame)
        self.notebook.add(self.log_frame, text="Log Output")

    # ... (Keep create_settings_tab) ...
    def create_settings_tab(self, frame):
        file_frame = ttk.LabelFrame(frame, text="File Settings", padding=(10, 5)); file_frame.grid(row=0, column=0, sticky="ew", padx=10, pady=5); file_frame.columnconfigure(1, weight=1)
        ttk.Label(file_frame, text="CSV Input File:").grid(row=0, column=0, sticky="w", pady=2, padx=5); ttk.Entry(file_frame, textvariable=self.csv_file, width=60).grid(row=0, column=1, sticky="ew", pady=2); ttk.Button(file_frame, text="Browse...", command=self.browse_csv).grid(row=0, column=2, padx=5, pady=2)
        ttk.Label(file_frame, text="Output XML (base name):").grid(row=1, column=0, sticky="w", pady=2, padx=5); ttk.Entry(file_frame, textvariable=self.xml_base, width=60).grid(row=1, column=1, sticky="ew", pady=2); ttk.Button(file_frame, text="Browse...", command=self.browse_xml).grid(row=1, column=2, padx=5, pady=2)
        ttk.Label(file_frame, text="CSV Report File (Optional):").grid(row=2, column=0, sticky="w", pady=2, padx=5); ttk.Entry(file_frame, textvariable=self.report_file, width=60).grid(row=2, column=1, sticky="ew", pady=2); ttk.Button(file_frame, text="Browse...", command=self.browse_report).grid(row=2, column=2, padx=5, pady=2)
        ttk.Button(file_frame, text="Load CSV Header -> Mapping Tab", command=self.populate_csv_mapping_tab).grid(row=4, column=1, sticky="w", pady=5)
        import_frame = ttk.LabelFrame(frame, text="Import Options", padding=(10, 5)); import_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=5); import_frame.columnconfigure(1, weight=1)
        ttk.Label(import_frame, text="Default Location Prefix:").grid(row=0, column=0, sticky="w", pady=2, padx=5); ttk.Entry(import_frame, textvariable=self.default_location, width=60).grid(row=0, column=1, sticky="ew", pady=2)
        ttk.Label(import_frame, text="Default Category (if unmapped):").grid(row=1, column=0, sticky="w", pady=2, padx=5); ttk.Entry(import_frame, textvariable=self.category, width=60).grid(row=1, column=1, sticky="ew", pady=2)
        ttk.Label(import_frame, text="Created By Override:").grid(row=2, column=0, sticky="w", pady=2, padx=5); ttk.Entry(import_frame, textvariable=self.username, width=60).grid(row=2, column=1, sticky="w", pady=2)
        ttk.Label(import_frame, text="Action Override:").grid(row=3, column=0, sticky="w", pady=2, padx=5); action_frame = ttk.Frame(import_frame); action_frame.grid(row=3, column=1, sticky="w");
        for a in ["none", "sync", "addversion", "delete", "update (metadata)"]: ttk.Radiobutton(action_frame, text=a.capitalize(), variable=self.action, value=a).pack(side="left", padx=5)
        self.action.set("sync")
        ttk.Label(import_frame, text="Default Node Type Override:").grid(row=4, column=0, sticky="w", pady=2, padx=5); type_frame = ttk.Frame(import_frame); type_frame.grid(row=4, column=1, sticky="w");
        for nt in ["none", "folder", "document"]: ttk.Radiobutton(type_frame, text=nt.capitalize(), variable=self.node_type, value=nt).pack(side="left", padx=5)
        self.node_type.set("folder")
        ttk.Label(import_frame, text="Batch Size (rows per XML):").grid(row=5, column=0, sticky="w", pady=2, padx=5); ttk.Entry(import_frame, textvariable=self.batch_size, width=10).grid(row=5, column=1, sticky="w", pady=2)
        check_frame = ttk.Frame(import_frame); check_frame.grid(row=6, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        ...
        advanced_frame = ttk.LabelFrame(frame, text="Advanced CSV Options", padding=(10, 5)); advanced_frame.grid(row=2, column=0, sticky="ew", padx=10, pady=5); advanced_frame.columnconfigure(1, weight=1)
        ttk.Label(advanced_frame, text="CSV Delimiter (blank=auto):").grid(row=0, column=0, sticky="w", pady=2, padx=5); ttk.Entry(advanced_frame, textvariable=self.csv_delimiter, width=5).grid(row=0, column=1, sticky="w", pady=2)
        ttk.Label(advanced_frame, text="CSV Quote Char (blank=auto):").grid(row=1, column=0, sticky="w", pady=2, padx=5); ttk.Entry(advanced_frame, textvariable=self.csv_quotechar, width=5).grid(row=1, column=1, sticky="w", pady=2)
        ttk.Label(advanced_frame, text="CDATA Fields (',' sep, '*' all):").grid(row=2, column=0, sticky="w", pady=2, padx=5); ttk.Entry(advanced_frame, textvariable=self.cdata_fields, width=60).grid(row=2, column=1, sticky="ew", pady=2)
        action_btn_frame = ttk.Frame(frame, padding=(10, 10)); action_btn_frame.grid(row=3, column=0, sticky="ew", padx=10, pady=5); action_btn_frame.columnconfigure(0, weight=1); action_btn_frame.columnconfigure(1, weight=1); action_btn_frame.columnconfigure(2, weight=1)
        ttk.Button(action_btn_frame, text="Start Generation", command=self.start_generation, width=20).grid(row=0, column=0, padx=10, pady=5)
        self.stop_button = ttk.Button(action_btn_frame, text="Stop Generation", command=self.stop_generation, width=20, state=tk.DISABLED); self.stop_button.grid(row=0, column=1, padx=10, pady=5)
        self.report_button = ttk.Button(action_btn_frame, text="View Status Report", command=self.view_status_report, width=20, state=tk.NORMAL if self.db_available else tk.DISABLED); self.report_button.grid(row=0, column=2, padx=10, pady=5)
        project_btn_frame = ttk.Frame(frame, padding=(10, 5)); project_btn_frame.grid(row=4, column=0, sticky="ew", padx=10, pady=5); project_btn_frame.columnconfigure(0, weight=1); project_btn_frame.columnconfigure(1, weight=1)
        ttk.Button(project_btn_frame, text="Open Project...", command=self.open_project, width=20).grid(row=0, column=0, padx=10, pady=5)
        ttk.Button(project_btn_frame, text="Save Project...", command=self.save_project, width=20).grid(row=0, column=1, padx=10, pady=5)

    # ... (Keep create_csv_mapping_tab and its helpers) ...
    def create_csv_mapping_tab(self, frame):
        ...
        ttk.Button(top_frame, text="Apply && Save Mapping Changes", command=self.save_csv_mapping_tab).pack(side="left", padx=5)
        canvas_frame = ttk.Frame(frame); canvas_frame.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        ...
        header_frame = ttk.Frame(self.csv_mapping_inner); header_frame.grid(row=0, column=0, sticky="ew")
        ...
        self.csv_mapping_entries = []; self.csv_mapping_rows_frame = ttk.Frame(self.csv_mapping_inner); self.csv_mapping_rows_frame.grid(row=1, column=0, sticky="ew")
    ...
    def populate_csv_mapping_tab(self):
        ...
        self.csv_mapping_entries.clear(); logging.info("Populating CSV mapping tab...")
        csv_path = self.csv_file.get()
        if not csv_path or not os.path.exists(csv_path): logging.warning("Cannot populate mapping tab: CSV file not selected or found."); return
        try:
        ...
        except StopIteration: logging.error(f"CSV file {csv_path} appears to be empty."); messagebox.showerror("CSV Error", f"Could not read header from CSV file:\n{csv_path}\nIt might be empty."); return
        except Exception as e: logging.exception(f"Error reading CSV header from {csv_path}"); messagebox.showerror("CSV Error", f"Unable to read CSV file header:\n{e}"); return
        row_idx = 0
        for col_header in headers:
            ...
            ttk.Label(self.csv_mapping_rows_frame, text=original_col, anchor="w").grid(row=row_idx, column=0, sticky="ew", padx=5, pady=2)
            cmb_map_type = ttk.Combobox(self.csv_mapping_rows_frame, values=["Ignore", "Standard", "Metadata"], width=12, state="readonly"); cmb_map_type.set(default_map.get("MappingType", "Metadata")); cmb_map_type.grid(row=row_idx, column=1, sticky="ew", padx=5, pady=2)
            ent_target = ttk.Entry(self.csv_mapping_rows_frame, width=20); ent_target.insert(0, default_map.get("TargetLabel", original_col)); ent_target.grid(row=row_idx, column=2, sticky="ew", padx=5, pady=2)
            cat_var = tk.StringVar(); cat_var.set(default_map.get("Category", "")); cat_display_entry = ttk.Entry(self.csv_mapping_rows_frame, textvariable=cat_var, state="readonly", width=18); cat_display_entry.grid(row=row_idx, column=3, sticky="ew", padx=5, pady=2)
            select_btn = ttk.Button(self.csv_mapping_rows_frame, text="...", width=3, command=lambda v=cat_var: self.open_category_selector(v)); select_btn.grid(row=row_idx, column=4, sticky="w", padx=(0,5), pady=2)
            self.csv_mapping_entries.append((norm_col, cmb_map_type, ent_target, cat_var)); row_idx += 1
        self.csv_mapping_canvas.update_idletasks(); self.csv_mapping_canvas.configure(scrollregion=self.csv_mapping_canvas.bbox("all")); logging.info(f"CSV mapping tab populated with {row_idx} entries.")
    def save_csv_mapping_tab(self):
        ...
        self.mapping = new_mapping; logging.info(f"CSV mapping updated and saved internally ({len(self.mapping)} rules)."); messagebox.showinfo("Mapping Updated", "CSV mapping rules have been updated.\nSave the project to persist these changes.")
    def open_category_selector(self, cat_var):
        win = tk.Toplevel(self); win.title("Select Categories"); win.geometry("350x350"); win.transient(self); win.grab_set()
        ttk.Label(win, text="Select one or more categories:").pack(pady=(10, 5))
        ...
        def on_ok(): selected_indices = cat_listbox.curselection(); selected_full_paths = [self.category_display_map[display_categories[i]] for i in selected_indices]; cat_var.set(",".join(selected_full_paths)); win.destroy()
    ...
    # ... (Keep create_special_mapping_tab and its helpers) ...
    ...
    # ... (Keep create_categories_tab and its helpers) ...
    ...
    # ... (Keep start_generation, _do_generation, _generation_complete, stop_flag_func, stop_generation) ...
    ...
    def _do_generation(self):
        ...
        try:
            ...
            updated_map = run_processing(
                ...
                force_reprocess=self.force_reprocess_var.get() # Pass force reprocess flag
            )
            success = True
        except Exception as e:
    ...
    def _generation_complete(self, success):
        ...
        if success: self.view_status_report() # Show report automatically on completion

    # ... (Keep view_status_report) ...
    def view_status_report(self):
        """Generates and displays the status report in the log window."""
    ...
    # --- Reprocess Tab Logic (Updated for _uncreated.xml workflow) ---
    def create_reprocess_tab(self, frame):
        top_frame = ttk.Frame(frame, padding=(10, 5)); top_frame.pack(fill="x")
        # *** UPDATED Button Text and Command ***
        ...
        self.reprocess_tree.heading("error", text="Import Error (from XML)"); self.reprocess_tree.column("error", width=300, anchor="w", stretch=True) # Clarified error source
        ...
        self.reprocess_entries = {} # Stores {treeview_iid: combined_data_dict}

    # *** RENAMED and REWRITTEN Function ***
    def load_uncreated_xml_and_prepare_reprocess(self):
        """
        Loads the _uncreated.xml file from Content Server import,
        matches failed items to the database, regenerates their XML,
        and populates the reprocess treeview.
        """
        if not self.db_available: messagebox.showerror("Database Error", "Database is not available."); return
        logging.info("Starting reprocess: Loading _uncreated.xml...")

        xml_path = filedialog.askopenfilename(
            filetypes=[("Uncreated XML", "*_uncreated.xml"), ("All Files", "*.*")],
            title="Select Content Server _uncreated.xml File"
        )
        if not xml_path:
            logging.info("Reprocess cancelled: No _uncreated.xml file selected.")
            return

        # --- Step 1: Parse _uncreated.xml ---
        failed_items_from_xml = []
        try:
            # Using the corrected parsing logic block
                    ...
                    if stripped_line.startswith("', stripped_line)
                        ...
                        if stripped_line.endswith("</node>"):
                            ...
                            try:
                                ...
                                identifier = title if title else location # Use title or location
                            except Exception as pe:
                                logging.warning(f"Could not parse node XML snippet from _uncreated.xml: {pe}")
                                identifier = "(Identifier unavailable)" # Cannot proceed without identifier

                            # Store identifier and the error message from the XML comment
                            if identifier != "(Identifier unavailable)":
                                failed_items_from_xml.append({
                                    'identifier': identifier,
                                    'xml_error': current_error # Store the error reported by Content Server
                                })
                            node_xml_lines = []
            logging.info(f"Parsed {len(failed_items_from_xml)} failed entries from {xml_path}")

        except Exception as e:
            logging.exception(f"Error parsing _uncreated.xml file: {xml_path}")
            messagebox.showerror("Parse Error", f"Error reading _uncreated.xml:\n{e}")
            return
        ...
        # --- Step 2: Match XML failures to DB and Regenerate ---
        logging.info("Matching failed items to database and regenerating nodes...")
        entries_for_treeview = []
        regen_errors = 0
        match_errors = 0

        if not self.mapping:
            messagebox.showerror("Error", "Cannot regenerate nodes: CSV Mapping is empty.")
            return # Need mapping to regenerate

        for failed_item in failed_items_from_xml:
            xml_identifier = failed_item['identifier']
            xml_error = failed_item['xml_error']

            db_object = db_handler.get_object_by_identifier(xml_identifier, self.current_db_path)

            if not db_object:
                logging.warning(f"Could not find matching DB record for identifier from XML: '{xml_identifier}'")
                match_errors += 1
                # Add placeholder to treeview indicating no match?
                entries_for_treeview.append({
                     'unique_id': '(No DB Match)', 'identifier': xml_identifier,
                     'error_message': xml_error, # Show original XML error
                     'generated_xml': None, 'action_state': 'Skip'
                 })
                continue

            unique_id = db_object.get('unique_id')
            csv_data = db_object.get('csv_data')
            row_index = db_object.get('csv_row_index', 'N/A')

            if not csv_data:
                 logging.error(f"Cannot regenerate node for {unique_id} (Identifier: {xml_identifier}): Original CSV data missing from database.")
                 entries_for_treeview.append({
                     'unique_id': unique_id, 'identifier': xml_identifier,
                     'error_message': 'Original CSV data missing in DB',
                     'generated_xml': None, 'action_state': 'Skip', 'db_data': db_object
                 })
                 regen_errors += 1
                 continue

            node_elem, error_msg = process_row(
                row_index=row_index, csv_data=csv_data, mapping=self.mapping,
                default_location=self.default_location.get(), username=self.username.get(),
                selected_action=self.action.get(), default_node_type=self.node_type.get(),
                category_default=self.category.get(), use_csv_createdby=self.use_csv_createdby.get(),
                report_dict=None, rename_list=[], special_map=self.special_char_map
            )

            tree_entry = {
                'unique_id': unique_id, 'identifier': xml_identifier,
                'error_message': xml_error, # Display the error from the _uncreated.xml file
                'generated_xml': ET.tostring(node_elem, encoding='unicode') if node_elem else None,
                'action_state': 'Re-import' if node_elem else 'Skip',
                'db_data': db_object # Keep original DB data if needed
            }
            entries_for_treeview.append(tree_entry)
            if node_elem is None:
                logging.error(f"Failed regenerating node for {unique_id} (Identifier: {xml_identifier}): {error_msg}")
                regen_errors += 1
                tree_entry['error_message'] = f"Regen Failed: {error_msg}" # Overwrite XML error with regen error

        # --- Step 3: Populate Treeview ---
    ...
    def populate_reprocess_tree(self, entries):
        """Clears and populates the Reprocess Treeview from prepared entry list."""
        for item_id in self.reprocess_tree.get_children(): self.reprocess_tree.delete(item_id)
        self.reprocess_entries.clear()
        self.generate_reprocess_button.config(state=tk.DISABLED)

        if not entries: logging.info("Reprocess tree: No entries to display."); return

        for entry_data in entries:
            unique_id = entry_data.get('unique_id', '(No ID)')
            identifier = entry_data.get('identifier', '(No Identifier)')
            error_msg = entry_data.get('error_message', '(No Error Msg)')
            action_state = entry_data.get('action_state', 'Skip')

            iid = self.reprocess_tree.insert("", "end", values=(unique_id, identifier, error_msg, action_state))
            entry_data['action_tkvar'] = tk.StringVar(value=action_state)
            self.reprocess_entries[iid] = entry_data
    ...
    def on_reprocess_tree_double_click(self, event):
        ...
        # Ensure item is eligible for re-import (check if generated_xml exists)
        ...
        if not can_reimport and current_action == "Re-import":
             action_var.set("Skip") # Force to Skip if cannot be re-imported

        combobox = ttk.Combobox(self.reprocess_tree, values=available_actions, state="readonly", textvariable=action_var)
        combobox.place(x=x, y=y, width=width, height=height)
        combobox.focus_set()

        def on_select_or_focus_out(event):
            new_val = action_var.get()
            logging.debug(f"Reprocess action for item '{self.reprocess_entries[row_id]['unique_id']}' set to '{new_val}'")
            combobox.destroy()
        def on_escape(event): combobox.destroy()

        combobox.bind("<<ComboboxSelected>>", on_select_or_focus_out)
        combobox.bind("<FocusOut>", on_select_or_focus_out)
        combobox.bind("<Escape>", on_escape)


    def generate_reprocess_xml(self):
        """Generates the final XML file for items marked 'Re-import'."""
        ...
        for iid, entry_data in self.reprocess_entries.items():
            if entry_data['action_tkvar'].get() == "Re-import":
                if entry_data.get('generated_xml'):
                    entries_to_export.append({
                        'unique_id': entry_data.get('unique_id'),
                        'generated_xml': entry_data.get('generated_xml'),
        ...
        try:
            ...
            if success:
                 ...
                 for uid in processed_ids:
                      # Update DB status, clear error message for reprocessed items
                 ...
                 # Clear tree after successful generation
                 for item in self.reprocess_tree.get_children(): self.reprocess_tree.delete(item)
                 self.reprocess_entries.clear(); self.generate_reprocess_button.config(state=tk.DISABLED)
            else: messagebox.showwarning("File Not Saved", "Reprocess XML could not be saved. See log.")
        except Exception as e:
            logging.exception(f"Error during reprocess XML generation: {out_path}")
            messagebox.showerror("Error", f"Failed to generate reprocess XML:\n{e}")

    # ... (Keep project load/save, closing, browse methods) ...
    def open_project(self):
        ...
        # DB Path Handling - For now, still using global DB_PATH
    ...
    def on_closing(self):
        ...
        if messagebox.askokcancel("Quit", "Do you really want to quit?"):
            ...
            self.destroy()
    def browse_csv(self): path = filedialog.askopenfilename(title="Select Input CSV File", filetypes=[("CSV Files", "*.csv"),("All Files", "*.*")]);
    if path: self.csv_file.set(path)
    def browse_xml(self): path = filedialog.asksaveasfilename(title="Select Output XML Base Path/Name", defaultextension=".xml", filetypes=[("XML Files", "*.xml"), ("All Files", "*.*")]);
    if path: self.xml_base.set(path)
    def browse_report(self): path = filedialog.askopenfilename(title="Select CSV Report File (Optional)", filetypes=[("CSV Files", "*.csv"), ("All Files", "*.*")]);
