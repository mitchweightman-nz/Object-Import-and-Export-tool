...
def convert_xml_to_csv(xml_string: str, selected_fields_by_node: dict | None = None) -> str:
    ...
    writer = csv.writer(output_buffer, quoting=csv.QUOTE_ALL, lineterminator='\n')

    if final_headers or processed_rows_data: # Only write header if there are headers or data to write
        writer.writerow(final_headers)

    for row_data in processed_rows_data:
        if any(h in row_data for h in final_headers) or (not final_headers and not row_data):
            row_to_write = [row_data.get(header, "") for header in final_headers]
            writer.writerow(row_to_write)
        elif not row_data and 'element_tag' in final_headers and len(final_headers) == 1:
             writer.writerow([""])
